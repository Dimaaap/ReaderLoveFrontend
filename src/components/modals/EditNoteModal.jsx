...
export const EditNoteModal = ({ username, note }) => {


    const queryClient = useQueryClient();
    const { editBookNoteModalOpen, setEditBookNoteModalOpen } = useEditBookNoteModalState();
    ...
    const { register, handleSubmit, control, watch, setValue, reset, formState: { errors } } = useForm({
        defaultValues: {
            ...
            book_page: note?.book_page?.toString(),
    ...
    const { mutate: updateNoteMutation } = useMutation({
        mutationFn: async(updatedNote) => {
            const response = await fetch(AllLinks.bookNotes.PARTIAL_UPDATE_NOTE(note.id), {
                method: "PATCH",
                headers: {
            ...
            if(!response.ok){
                ...
                throw new Error(errorData.message || "Помилка при оновленні нотатки")
            }
        ...
        onSuccess: async () => {
            const currentUsername = username || note?.user_username || note?.user?.username;

            console.log("Invalidating key for user:", currentUsername);

            await queryClient.invalidateQueries({
                queryKey: ["notes", currentUsername],
            });
            ...
            setEditBookNoteModalOpen(false);
        },
    ...
    const onSubmitForm = data => {
        ...
        updateNoteMutation(formData)
    }

    if(!editBookNoteModalOpen) return null

    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
        onClick={ () => setEditBookNoteModalOpen(false) }>
            ...
            onClick={ (e) => e.stopPropagation() } onSubmit={ handleSubmit(onSubmitForm) }>
                ...
                <div className="flex flex-col gap-1 justify-end">
                    <textarea className="rounded-lg border border-zinc-600 p-3 text-white text-lf font-semibold w-full active:border-[#F43F5E]"
                    { ...register("note_text", {
                ...
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                        <button className="text-white font-semibold text-sm cursor-pointer transition-all duration-200 hover:opacity-80" type="button"
                        disabled={ updateNoteMutation.isPending }
                        onClick={ () => setEditBookNoteModalOpen(false) }>
                            Скасувати
                        </button>
                        <button className="bg-[#F43F5E] text-white font-semibold text-sm p-2 rounded-lg cursor-pointer transition-all duration-200 hover:opacity-80" type="submit"
                        disabled={ updateNoteMutation.isPending }>
                            {updateNoteMutation.isPending ? "Збереження..." : "Зберегти зміни"}
                        </button>
                    </div>
                </div>
